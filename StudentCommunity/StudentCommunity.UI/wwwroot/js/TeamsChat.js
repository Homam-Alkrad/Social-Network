$(document).ready(function () {
    ...
    connection.start().then(function () {
        ...
        viewModel.roomList(roomIdFromModel);
        viewModel.userList();
    }).catch(function (err) {
    ...
    function AppViewModel() {
        ...
        self.roomList = function (defaultRoomId) {
            fetch('/api/Rooms')
                .then(response => response.json())
                .then(data => {
                    self.chatRooms.removeAll();

                    // Find the default room in the fetched data
                    var defaultRoomData = data.find(room => room.id === defaultRoomId);

                    // Add the default room to chatRooms if found
                    if (defaultRoomData) {
                        self.chatRooms.push(new ChatRoom(defaultRoomData.id, defaultRoomData.name, defaultRoomData.admin));
